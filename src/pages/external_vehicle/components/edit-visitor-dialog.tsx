// src/pages/external_vehicle/components/edit-visitor-dialog.tsx - ไฟล์เต็มที่แก้ไขแล้ว
"use client";
...
export function EditVisitorDialog({
  ...
  // Safe function to parse array data - แก้ไขให้ robust มากขึ้น
  const safeParseArray = (data: any): string[] => {
    if (Array.isArray(data)) {
      return data.filter((item) => typeof item === "string"); // เฉพาะ string เท่านั้น
    }
  ...
  // Reset form when visitorData changes - แก้ไขให้ safe
  useEffect(() => {
    if (visitorData && open) {
      console.log("=== DEBUG VISITOR DATA ===");
      console.log("Full visitorData:", JSON.stringify(visitorData, null, 2));
      console.log("Vehicle data:", visitorData.vehicle);
      console.log("Vehicle type:", typeof visitorData.vehicle);

      // เช็คว่า vehicle มีข้อมูลหรือไม่
      if (!visitorData.vehicle) {
        console.warn("⚠️ Warning: visitorData.vehicle is undefined/null");
        toast.warning("ข้อมูลยานพาหนะไม่สมบูรณ์", {
          description: "กรุณาตรวจสอบข้อมูลยานพาหนะ",
        });
      }
      ...
      try {
        ...
        // สร้าง default vehicle object ถ้าไม่มี
        ...
        const formData = {
          ...
          gender:
            (visitorData.gender as "male" | "female" | "other") || "other",
          id_card: visitorData.id_card || "",
          // ใช้ default vehicle object
  ...
  async function onSubmit(values: EditFormSchema) {
    ...
    try {
      ...
      console.log("Original visitor data:", visitorData);
      ...
      // ตรวจสอบ visitorData ก่อนใช้งาน
      if (!visitorData?.id) {
        throw new Error("ไม่พบข้อมูลผู้เยี่ยมที่ต้องการแก้ไข");
      }
      ...
      console.log("Sending API request:", reqData);
  ...
  return (
    <FormDialog
      ...
      description={`แก้ไขข้อมูล ${visitorData?.first_name || ""} ${visitorData?.last_name || ""} (${visitorData?.vehicle?.license_plate || "ไม่ระบุ"})`}
      isLoading={isLoading}
      isDirty={isDirty}

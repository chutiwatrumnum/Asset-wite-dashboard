...
import { CreateResidentDrawer } from "./components/create-resident-dialog";
...
import {
  useResidentAllListQuery,
  useBulkDeleteResidentMutation,
} from "@/react-query/manage/resident";
...
export default function Residents() {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      console.log("Deleting residents with IDs:", selectedIds);

      await bulkDeleteResident(selectedIds);
  ...
  const table = useReactTable({
    initialState: {
      columnVisibility: {
        id: false,
        // email: true, // แน่ใจว่า email แสดง
      },
    },
    data: data ?? [],
    columns: [
    ...
    <div className="w-full pl-10 pr-10">
      <Card className="mb-6">
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          ...
          <CreateResidentDrawer onResidentCreated={refetch} />
        </CardHeader>
        <CardContent>
          <p className="text-muted-foreground">
            จัดการข้อมูลลูกบ้านทั้งหมดในระบบ เพิ่ม แก้ไข หรือลบข้อมูลลูกบ้าน
          </p>
        </CardContent>
      </Card>
      ...
      <AlertDialog
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              คุณแน่ใจหรือไม่ที่จะลบลูกบ้าน {Object.keys(rowSelection).length}{" "}
              คน?
              <br />
